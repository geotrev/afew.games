...
![Pokemon UltraMoon manufacturing and product info, highlighting the satellite and part codes.](/uploads/pokemon-ultra-moon-revision.jpg)

Although you can't see the rest of the game, this is a standard retail version of _Pokemon UltraMoon_. Based on the info we already talked about, the B on the part code denotes this as a revision, aka the second printing of the retail copy. The 1 on the satellite code confirms that as well, in addition to its country of manufacture (Japan) and distribution (USA).
...
Something interesting of note on GameCube is that these part codes didn't start until shortly after GameCube's launch. That's why you'll see release titles, such as Super Smash Bros. Melee, have only the satellite code.

GameCube part codes are sometimes to the far left on the bottom, but occasionally are located right above the main UPC, under the "WARNING" label.
...
![Nintendo Wii print details with part code on the left of the UPC, and satellite code on the right.](/uploads/skyward-sword.jpg)
...
_Super Smash Bros Ultimate_ for Switch, retail first print, Made in Japan, distributed in Canada (see `CAN` MPN suffix). Notice that the part code here ends with `2A`, whereas on the USA print, it ends with `1A`.
...
- Third parties don't always respect the part code patterns on their first party game counterparts
...
![Shows third-party published game missing satellite code and revision suffix on part code](/uploads/kingdom-hearts.jpg)

On _Kingdom Hearts: 358/2 Days_ for DS, third party publisher Square Enix omitted the revision suffix in the part code and entirely omitted the satellite code.
...
On _Metroid Prime: Hunters_ for DS, Nintendo never printed a version of the game with the part code `57096A`, which means `57096B` is the "first" print. As far as I have found, the "A" code doesn't exist. Again, likely an error during production that was caught quickly and thrown out.
...
![Boxed Wii U game bundle has locale text and part code, but no satellite code](/uploads/super-smash-bros-wii-u-bundle.jpg)

On _Super Smash Bros_ for Wii U (the controller bundle version) there is no satellite code, however the game on the inside _will_ have a satellite code, with part code 83665A. Since it's a bundle it'll also say "Not For Resale," of course.
...
Lastly, one more tip: I recommend perusing eBay to compile a list of part codes/variant data. There is a high chance the game you're interested in researching has dozens, if not hundreds of online listings.

(09/09/2022) Correction: changed introduction to note part codes started with GameCube.

(09/19/2022) Correction: specifies the correct pack-in part code for Super Smash Bros. for Wii U; Addition: adds GameCube game example and explainer; Addition: "Gotchas" section for first prints.
